...
class AdvancedRag:
    def __init__(
        ...
        if llm_provider is None:
            ...
            self._llm_provider = create_provider(
                "openai", api_key, self._model, lambda n, a: ""
            )
        else:
    ...
    def generate_queries(self, question: str, max_queries: int = 4) -> list[str]:
        ...
        response_text = self._llm_provider.run(
            user_message=user, tools=[], system_prompt=system
        )
    ...
    def rerank(
        ...
        response_text = self._llm_provider.run(
            user_message=user, tools=[], system_prompt=prompt
        )
        text = response_text or ""
        data = _safe_json_loads(text)
        ranked_ids = (
